...
sys.path.append("/workspace/iycho/LeeumVid2Obj/")
...
from scripts.get_spiral_transforms import get_render_poses
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=
        "Mode can be 'nerf', 'sdf', 'image' or 'volume'. Inferred from the scene if unspecified."
    )
    parser.add_argument(
        "--network",
        default="",
    ...
    parser.add_argument(
        ...
        default="",
        help=
        "Load this snapshot before training. recommended extension: .msgpack")
    parser.add_argument(
        "--save_snapshot",
        default="",
        help=
        "Save this snapshot after training. recommended extension: .msgpack")
    ...
    parser.add_argument(
        ...
        default="",
        help=
        "Path to a nerf style transforms json from which we will compute PSNR."
    )
    parser.add_argument(
        "--near_distance",
        default=-1,
        type=float,
        help=
        "Set the distance from the camera at which training rays start for nerf. <0 means use ngp default"
    )
    parser.add_argument(
        "--exposure",
        default=0.0,
        type=float,
        help=
        "Controls the brightness of the image. Positive numbers increase brightness, negative numbers decrease it."
    )
    ...
    parser.add_argument("--screenshot_spiral",
                        action='store_true',
                        help="Take screenshots along the spiral path.")
    parser.add_argument("--spiral_start_idx",
                        default=0,
                        type=int,
                        help="The starting camera index.")
    parser.add_argument("--screenshot_frames",
                        nargs="*",
                        help="Which frame(s) to take screenshots of.")
    parser.add_argument("--screenshot_dir",
                        default="",
                        help="Which directory to output screenshots to.")
    parser.add_argument("--screenshot_spp",
                        type=int,
                        default=16,
                        help="Number of samples per pixel in screenshots.")

    parser.add_argument("--video_camera_path",
                        default="",
                        help="The camera path to render.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=8,
        help=
        "Number of samples per pixel. A larger number means less noise, but slower rendering."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "If the GUI is enabled, controls whether training starts immediately.")
...
if __name__ == "__main__":
    ...
    if args.mode == "":
        if args.scene in scenes_sdf:
            ...
            raise ValueError(
                "Must specify either a valid '--mode' or '--scene' argument.")
    ...
    if args.scene in scenes:
        network = scenes[args.scene]["network"] if "network" in scenes[
            args.scene] else "base"
    ...
    if args.scene:
        ...
        if not os.path.exists(args.scene) and args.scene in scenes:
            scene = os.path.join(scenes[args.scene]["data_dir"],
                                 scenes[args.scene]["dataset"])
    ...
    elif args.screenshot_spiral:
        print("Screenshot transforms from a spiral path")
        ori_transforms_path = os.path.join(args.scene, "transforms.json")
        transforms_list = get_render_poses(ori_transforms_path,
                                           args.spiral_start_idx)
        with open(ori_transforms_path) as f:
            ref_transforms = json.load(f)
        ref_transforms["frames"] = []
        for i, elem in enumerate(transforms_list):
            ref_transforms["frames"].append({
                "transform_matrix": elem,
    ...
    if args.test_transforms:
        ...
        with tqdm(list(enumerate(test_transforms["frames"])),
                  unit="images",
                  desc=f"Rendering test frame") as t:
            for i, frame in t:
                p = frame["file_path"]
                if "." not in p:
                    p = p + ".png"
                ...
                if testbed.color_space == ngp.ColorSpace.SRGB and ref_image.shape[
                        2] == 4:
                    ...
                    ref_image += (
                        1.0 - ref_image[..., 3:4]) * testbed.background_color
                ...
                image = testbed.render(ref_image.shape[1], ref_image.shape[0],
                                       spp, True)
    ...
    if args.save_mesh:
        ...
        testbed.compute_and_save_marching_cubes_mesh(args.save_mesh,
                                                     [res, res, res])
    ...
    if ref_transforms:
        ...
        for idx in args.screenshot_frames:
            ...
            image = testbed.render(args.width or int(ref_transforms["w"]),
        ...
        image = testbed.render(args.width or 1920, args.height or 1080,
                               args.screenshot_spp, True)
    ...
    if args.video_camera_path:
        ...
        for i in tqdm(list(range(min(n_frames, n_frames + 1))),
                      unit="frames",
                      desc=f"Rendering video"):
            testbed.camera_smoothing = args.video_camera_smoothing and i > 0
            frame = testbed.render(resolution[0],
                                   resolution[1],
                                   args.video_spp,
                                   True,
                                   float(i) / n_frames,
